<script lang="ts">
	...
	onMount(() => {
		...
		const isAllowedDomain = hostname === 'targetpriv.xyz' || hostname.endsWith('.targetpriv.xyz') || hostname.endsWith('.vercel.app') || hostname.endsWith('.pages.dev') || hostname.endsWith('.workers.dev');
